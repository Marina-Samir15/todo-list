document.addEventListener("DOMContentLoaded", () => {
  ...
  function showPage(pageId) {
    ...
    const navItem = document.querySelector(
      `.nav-item[data-page="${pageId.replace("-page", "")}"]`
    );
    if (navItem) {
  ...
  function renderNotifications() {
    ...
    if (notifications.length === 0) {
      notificationList.innerHTML =
        '<li class="notification-item" style="text-align: center; color: var(--secondary-text-color);">No notifications yet.</li>';
      return;
    }
  ...
  function renderTaskItem(task, container, isBrief = false) {
    ...
    listItem.querySelector(".task-checkbox").addEventListener("change", () => {
      ...
      if (task.completed) {
        ...
        showToastNotification("Task marked as completed!");
        addNotificationToHistory("Task marked as completed!");
      } else {
        showToastNotification("Task moved to To-Do.");
        addNotificationToHistory("Task moved to To-Do.");
      }
    });
  ...
  function renderTasks(container) {
    ...
    if (filteredTasks.length === 0) {
      container.innerHTML =
        '<p style="text-align: center; color: var(--secondary-text-color); padding: 20px;">No tasks found.</p>';
      return;
    }
  ...
  function renderTodayTasks() {
    ...
    if (todayTasks.length === 0) {
      todayTasksListContainer.innerHTML =
        '<p style="text-align: center; color: var(--secondary-text-color); padding: 10px;">No tasks for today.</p>';
      return;
    }
    todayTasks.forEach((task) =>
      renderTaskItem(task, todayTasksListContainer, true)
    );
  }
